# 🏫 ENSAO – National School of Applied Sciences of Oujda

**Mohammed First University – Oujda, Morocco**

Welcome to the GitHub organization of **ENSAO (École Nationale des Sciences Appliquées d’Oujda)**.  
This space brings together **engineering students, developers, and innovators** to collaborate, share projects, and promote open-source development across all ENSAO disciplines.

---

## 🎯 Mission
To build a collaborative ecosystem where ENSAO students can:
- Develop and share innovative engineering projects  
- Learn and grow through teamwork and experimentation  
- Contribute to open-source initiatives  
- Highlight the excellence and creativity of ENSAO’s community  

---

## 🧠 Fields of Study
ENSAO offers a diverse range of engineering programs, combining technology, innovation, and applied sciences:

- 🌐 **Information Technology & Communication Networks Engineering (ITIRC)**  
- 🔐 **Computer Security & Cybersecurity (SICS)**  
- 🤖 **Embedded Systems, Computer Sciences & Network Engineering (GSEIR)**  
- 📊 **Data Science & Cloud Computing Engineering (IDSCC)**  
- 👨🏻‍💻 **Computer Engineering (GINF)**  
- 🏗️ **Civil Engineering (GCIV)**  
- ⚡ **Electrical Engineering (GELC)**  
- 🏭 **Industrial Engineering (GIND)**  
- 💼 **Information Systems Management & Governance (MGSI)**  

---

## 🚀 Projects
Explore our repositories to find:
- Student and research projects  
- Clubs and innovation lab works  
- Tools, libraries, and open-source resources  

> Contributions are welcome!  
> Fork, star ⭐, or open a pull request to collaborate with the ENSAO community.

---

## 👥 Community
This organization connects ENSAO students, alumni, and collaborators interested in open innovation and technology.

- 🌐 [Official ENSAO Website](https://ensao.ump.ma)  
- 📧 Contact: ensao.community@gmail.com
